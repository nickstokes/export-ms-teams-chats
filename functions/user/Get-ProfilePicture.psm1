...
function Get-ProfilePicture ($userId, $assetsFolderPath, $clientId, $tenantId) {
    ...
    elseif (($null -eq $userId) -or ($attempted.ContainsKey($userId))) {
        ...
        "data:image/jpeg;base64,$($defaultProfilePicture)"
    }
    else {
        # if never attempted
        ...
        try {
            ...
            [IO.File]::WriteAllBytes($profilePictureFile, [Convert]::FromBase64String($defaultProfilePicture))
            "assets/$userId.jpg"
        }
    }
}

function Get-ProfilePictureAttempted($attempted) {
    return $attempted
}